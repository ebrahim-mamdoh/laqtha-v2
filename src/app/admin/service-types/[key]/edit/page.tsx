...
import { useServiceType, useUpdateServiceType } from '@/hooks/useAdmin';
...
export default function EditServiceTypePage() {
  ...
  // Initialize form data when service type loads
  useEffect(() => {
    if (serviceType) {
      setFormData({
        key: serviceType.key,
        icon: serviceType.icon,
        label: { ar: serviceType.label.ar, en: serviceType.label.en || '' },
        itemLabel: { ar: serviceType.itemLabel.ar, en: serviceType.itemLabel.en || '' },
        commissionRate: serviceType.commissionRate,
      });
      setFields(serviceType.fields || []);
      setItemFields(serviceType.itemFields || []);
    }
  }, [serviceType]);
    ...
    return (
      <AdminLayout>
        <div className={styles.loadingContainer}>
          <Spinner size="lg" />
          <p>جاري تحميل بيانات نوع الخدمة...</p>
        </div>
      </AdminLayout>
    );
  }
    ...
    <AdminLayout>
      <div className={styles.page}>
        ...
        <nav className={styles.breadcrumb}>
          ...
          <Link href={`/admin/service-types/${key}`}>{serviceType.label.ar}</Link>
          <span className={styles.breadcrumbSeparator}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M15 18l-6-6 6-6" />
            </svg>
          </span>
          <span className={styles.breadcrumbCurrent}>تعديل</span>
        </nav>

        {/* Header */}
        <div className={styles.header}>
          <div className={styles.headerInfo}>
            <span className={styles.typeIcon}>{formData.icon}</span>
            <div className={styles.headerText}>
              <h1>تعديل {serviceType.label.ar}</h1>
        ...
        <form onSubmit={handleSubmit}>
              ...
              <div className={styles.basicForm}>
                <div className={styles.formGrid}>
                  ...
                  <div className={styles.formGroup}>
                    ...
                    />
                    <span className={styles.helpText}>لا يمكن تعديل المعرف بعد الإنشاء</span>
                  </div>
          ...
          <div className={styles.formActions}>
            <Link href={`/admin/service-types/${key}`}>
              <Button type="button" variant="outline">
                إلغاء
              </Button>
            </Link>
            <Button type="submit" loading={updateServiceType.isPending}>
              حفظ التغييرات
            </Button>
          </div>
        </form>
